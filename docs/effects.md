...
> **v0.8.0 note:** Effects are selected with near-uniform probability (not weighted). The procedural palette (continuous warmth/saturation) is now the default color system; named schemes remain available as overrides. The emotion system now has 34 anchors (expanded from 26).
...
| 参数 | 范围 | 默认值 | 说明 |
|------|------|--------|------|
| `frequency` | 0.005-0.35 | 0.05 | 波纹密度 |
| `speed` | 0.1-7.0 | 1.0 | 动画速度 |
| `color_phase` | 0.0-1.0 | 0.0 | 颜色偏移 |
| `warmth` | 0.0-1.0 | — | 连续色温（柔性管线） |
| `saturation` | 0.0-1.0 | — | 连续饱和度（柔性管线） |
| | | | **变形参数（Deformation）** |
| `self_warp` | 0.0-1.0 | 0.0 | 自扭曲（输出反馈坐标偏移） |
| `noise_injection` | 0.0-1.0 | 0.0 | 噪声注入（坐标随机偏移） |
...
| `frequency` | 0.005-0.35 | 0.1 | 基础频率 |
| `amplitude` | 0.5-3.0 | 1.0 | 波幅 |
| `speed` | 0.1-7.0 | 1.0 | 动画速度 |
| `color_scheme` | — | `"ocean"` | 配色方案 |
| | | | **变形参数（Deformation）** |
| `self_warp` | 0.0-1.0 | 0.0 | 自扭曲（波值反馈坐标偏移） |
| `noise_injection` | 0.0-1.0 | 0.0 | 噪声注入（坐标随机偏移） |
...
| | | | **变形参数（Deformation）** |
| `distortion` | 0.0-1.0 | 0.0 | 极坐标噪声扭曲 |
| `multi_center` | 1-4 | 1 | 干涉中心点数量 |
...
| `rotation_speed` | 0.1-7.0 | 1.0 | 旋转速度 |
| `light_x/y/z` | — | 0, 1, -1 | 光源方向分量 |
| | | | **变形参数（Deformation）** |
| `surface_noise` | 0.0-1.0 | 0.0 | 表面噪声变形量 |
| `asymmetry_x` | 0.3-2.0 | 1.0 | X 轴非均匀缩放 |
| `asymmetry_y` | 0.3-2.0 | 1.0 | Y 轴非均匀缩放 |
| `twist` | 0.0-2.0 | 0.0 | 截面扭曲 |
| `count` | 1-3 | 1 | 环面数量 |
| `ring_offset` | 0.1-0.5 | 0.3 | 多环面轨道偏移 |
...
| | | | **变形参数（Deformation）** |
| `vertex_noise` | 0.0-1.0 | 0.0 | 顶点噪声位移 |
| `morph` | 0.0-1.0 | 0.0 | 形态变形系数 |
...
`procedural/compositor.py` 同时提供 `MaskedCompositeEffect`，使用空间遮罩（`procedural/masks.py`）在不同区域混合两个效果：
